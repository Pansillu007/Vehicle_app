...
<!-- Responsive Navigation Menu -->
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-white/90 dark:bg-slate-800/90 backdrop-blur-xl border-t border-gray-200/50 dark:border-white/[0.06]">
        <div class="pt-2 pb-3 space-y-1 px-3">
            ...
            <a href="{{ route('trash.index') }}" class="flex items-center px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 {{ request()->routeIs('trash.*') ? 'text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-500/10' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5' }}">
                Trash
            </a>
            @if(auth()->user()->isAdmin())
            <a href="{{ route('admin.dashboard') }}" class="flex items-center px-4 py-2.5 rounded-xl text-sm font-medium {{ request()->routeIs('admin.*') ? 'text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-500/10' : 'text-gray-600 dark:text-gray-300' }}">
                Admin
            </a>
            @endif
        </div>
